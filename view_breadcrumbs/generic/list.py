...
class ListBreadcrumbMixin(BaseModelBreadcrumbMixin):
    ...
    def list_view_name(self):
        return action_view_name(model=self.model, action=self.list_view_suffix, app_name=self.app_name, full=False)

    @property
    def __list_view_name(self):
        return action_view_name(model=self.model, action=self.list_view_suffix, app_name=self.app_name)
