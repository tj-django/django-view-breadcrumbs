...
class CreateBreadcrumbMixin(ListBreadcrumbMixin):
    ...
    def create_view_name(self):
        return action_view_name(model=self.model, action=self.create_view_suffix, app_name=self.app_name, full=False)

    @property
    def __create_view_name(self):
        return action_view_name(model=self.model, action=self.create_view_suffix, app_name=self.app_name)
